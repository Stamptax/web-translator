...
let selectionInProgress = false; // Selection state tracking variable
...
function showSelectionIcon() {
  try {
    ...
    Object.assign(selectionIcon.style, {
      position: "fixed",
      top: `${lastMousePosition.y - 30}px`,
      left: `${lastMousePosition.x + 5}px`,
      width: "28px",
      height: "28px",
...
// Handle document click - remove UI elements if clicking outside
...
// Get selected text and send for translation
function translateSelectedText() {
  console.log("Starting translation of selected text");
  if (!isTranslating) {
    isTranslating = true;

    // Show loading prompt
    showTranslationBox(uiTexts[interfaceLanguage].translating, true);

    const selection = window.getSelection();
    const selectedText = selection.toString().trim();

    // Get text structure for format preservation
    const structure = getSelectionStructure(selection);
    console.log("Text structure:", structure ? "preserved" : "not preserved");
        ...
        if (response && response.success) {
          ...
          showTranslationBox(
            (response && response.error) ||
              uiTexts[interfaceLanguage].translationFailed,
            false
          );
        }
        isTranslating = false;
...
// Get the selection structure to preserve format
function getSelectionStructure(selection) {
  try {
    ...
    // Check if fragment contains elements
    return fragment.childNodes.length > 1 ? fragment : null;
  } catch (error) {
    console.error("Error getting selection structure:", error);
    return null;
  }
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  switch (message.action) {
    case "showTranslation":
      showTranslationBox(
        message.translation,
        message.isLoading !== undefined ? message.isLoading : false
      );
    ...
    case "showLoading":
      showTranslationBox(
        message.translation || uiTexts[interfaceLanguage].translating,
        true
      );
      if (sendResponse) sendResponse({ success: true });
      break;
...
// Position translation box
function positionTranslationBox() {
  // Implementation already handled in showTranslationBox
}
